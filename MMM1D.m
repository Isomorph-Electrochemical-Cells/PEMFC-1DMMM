...
T_A = T_0+90; % [K] temperature of anode bipolar plate and gas channel
T_C = T_0+90; % [K] temperature of cathode bipolar plate and gas channel
...
function dydx = odefun(x, y, subdomain)
    ...
    switch subdomain
        ...
        case 2 % ACL
            ...
            dj_H2O = -S_ad-x_H2O.*(-S_ad-S_F); % conservation of water vapor: div(j_H2O) = S_H2O + w_H2O*(S_H2O+S_H2)
        case 3 % PEM
...
function y0 = yinit(x, subdomain)
    ...
    y0 = [phi_e; 0; phi_p; 0; T; 0; lambda; 0; x_H2O; 0; x_O2; 0; s; 0];
...
function res = bcfun(ya, yb, U)
    ...
    res(8*Neq+11) = yb(11,5) - x_O2_C; % cathode GC oxygen content
